// based on SpecificRate_dataID1.C
// only production of results per fill
// remove of final plot: eff versus int lumi

// declarations

void SpecificFill( int fill, string filename,double results[49][2]);
std::vector<int> AnalysedFills();
bool IsNewFill(int fill);

bool useLumiNorm_;
int dataId_;

void SpecificRatev2( bool useLumiNorm = false, int dataId = 1 )
{
   TString wdir = gSystem->WorkingDirectory();
   TString ddir = "dataid"+TString::Itoa(dataId,10);
   TString ndir = ( useLumiNorm ) ? "luminorm" : "n1n2norm";
   TString odir = ddir + "/" + ndir + "/";
   
   if ( ! gSystem->ChangeDirectory(ddir) )
   {
      TString command = "mkdir -p " + odir;
      gSystem -> Exec(command);
   }
   else
   {
      if ( ! gSystem->ChangeDirectory(ndir) )
      {
         TString command = "mkdir -p " + ndir;
         gSystem -> Exec(command);
      }
   }
   gSystem->ChangeDirectory(wdir+"/"+odir);
   cout << gSystem->WorkingDirectory() << endl;
   
   useLumiNorm_ = useLumiNorm;
   dataId_ = dataId;
   
   std::vector<int> fillsDone = AnalysedFills();
   
   if ( useLumiNorm_ )
      std::cout << "* Info * Using luminosity as normalisation" << std::endl;
   else
      std::cout << "* Info * Using product of bunch intensities as normalisation" << std::endl;
   
   if ( dataId_ == 2 )
      std::cout << "* Info * Using data id 2 (aka lumi bins)" << std::endl;
   else
      std::cout << "* Info * Using data id 1" << std::endl;
      
   const char * dirname = "/nfs/dust/cms/user/walsh/bril/bcm1f/rhu/v3/";
   string filename,fillnumber;
   TSystemDirectory dir(dirname, dirname);
   TList *files = dir.GetListOfFiles();
   double results[49][2];
   double x [49][100]; // Larger size than number of fills
   double y [49][100];
   double yerr [49][100];
   double xerr [49][100];
   double NORM [49];
   
    if (files)
    {
        TSystemFile *file;
        TString fname;
        TIter next(files);
        while ((file=(TSystemFile*)next())) {
            fname = file->GetName();
            if (!file->IsDirectory() && fname.BeginsWith("rhu_") && fname.EndsWith(".root"))
            {
               int fillnr = TString(fname(4,4)).Atoi();
               filename  = string(dirname)+string(fname.Data());
               if ( std::find(fillsDone.begin(), fillsDone.end(), fillnr) == fillsDone.end() )
               {
                  std::cout << fillnr << "  will be processed" << std::endl;
                  SpecificFill(fillnr,filename,results);
               }
            }
        }
    }
      
}


// ========================================================



void SpecificFill( int fill, string filename,double results[49][2])
{
   
   static const int nbx = 3564;
   static const int ntrains = 100;  // a large number, larger than usual number of trains
   
   TFile * f = new TFile(filename.c_str(), "read");;
   
   // Filling scheme
   TTree * t_info = (TTree*) f -> Get("beaminfo");
   int bxcfg1[nbx];
   int bxcfg2[nbx];
   t_info->SetBranchAddress("bxconfig1", bxcfg1);
   t_info->SetBranchAddress("bxconfig2", bxcfg2);
   t_info -> GetEntry(0);
   
   std::vector<int> colliding; // array containing all colliding bunch crossings
   std::vector<int> bx; // array with first colliding bunch crossing in the train (better name needed)
   for ( size_t ii = 0; ii < nbx ; ++ii)
   {
       if ( bxcfg1[ii] == 1 && bxcfg2[ii] == 1 )  // colliding bunches
       {
           colliding.push_back(ii);
           if ( ii > 0 && ii < nbx-1 )
           {
               if ( bxcfg1[ii-1] == 0 )
               {
                   bx.push_back(ii);
//                    cout << "First bunch in train: " << ii << endl;
               }
           }
       }
   }
   
   cout << "Fill " << fill << " --> Number of trains = " << bx.size() << endl;
   
   // RHU data
   TTree * t = (TTree*) f -> Get("rhu");
   
   int time;
   float data[171072];
   float bxi1[3564];
   float bxi2[3564];
   float bxlumi[3564];
   
   t->SetBranchAddress("time", &time);
   if ( dataId_ == 2 )
      t->SetBranchAddress("dataid2", data);
   else
      t->SetBranchAddress("dataid1", data);
   t->SetBranchAddress("bxi1", bxi1);
   t->SetBranchAddress("bxi2", bxi2);
   t->SetBranchAddress("bxlumi",bxlumi);
   
   // Create histograms for rates and intensities
   TH1F * h_spec_rate_lumi[ntrains];
   TH1F * h_spec_rate_raw[ntrains];
   TH1F * h_spec_rate_ch[48][ntrains];
   TH1F * h_bxi1[ntrains];
   TH1F * h_bxi2[ntrains];
   TH1F * h_bxlumi[ntrains];
   for ( size_t ii = 0; ii < bx.size(); ++ii )
   {
      if ( useLumiNorm_ )
         h_spec_rate_lumi[ii] = new TH1F(Form("h_spec_rate_lumi_%i",bx[ii]),"",1600,0,1600);
      else
         h_spec_rate_lumi[ii] = new TH1F(Form("h_spec_rate_lumi_%i",bx[ii]),"",600,0,60);
      
      h_bxi1[ii]  = new TH1F(Form("h_bxi1_%i",bx[ii]),"",2000,0,20);
      h_bxi2[ii]  = new TH1F(Form("h_bxi2_%i",bx[ii]),"",2000,0,20);
      h_bxlumi[ii]= new TH1F(Form("h_bxlumi_%i",bx[ii]),"",2000,2,7);
      for ( int j =0 ; j < 48 ; ++j )
      {
         if ( useLumiNorm_ )
            h_spec_rate_ch[j][ii] = new TH1F(Form("h_spec_rate_ch_%i_%i",j,bx[ii]),"",1600,0,1600);
         else
            h_spec_rate_ch[j][ii] = new TH1F(Form("h_spec_rate_ch_%i_%i",j,bx[ii]),"",600,0,60);
      }
  }
   
   // Loop over events
   for ( int i = 0; i < t->GetEntries(); ++i )
   {
       t -> GetEntry(i);
       
       for ( size_t j = 0 ;j < bx.size(); ++j)
       {
           float i1 = bxi1[bx[j]]/1.E10;
           float i2 = bxi2[bx[j]]/1.E10;
           float lumi = bxlumi[bx[j]];
           float specNorm = lumi;
           if ( ! useLumiNorm_ ) specNorm = i1*i2;
           h_bxi1[j] -> Fill(i1);
           h_bxi2[j] -> Fill(i2);
           h_bxlumi[j]->Fill(lumi);
           float rate[48];
           for ( int k = 0; k < 48 ; ++k )
           {
               rate[k] = data[(k*3564)+bx[j]];
               h_spec_rate_ch[k][j]->Fill(rate[k]/specNorm);
           }
           float totalRate = rate[40]+rate[41]+rate[44]+rate[45];
           h_spec_rate_lumi[j]->Fill(totalRate/specNorm);
       }
   }
   
   TFile * fout = new TFile(Form("analysis_perch_%i.root",fill),"recreate");
   
   double gbx[ntrains];
   double gbxe[ntrains];
   double mean[ntrains];
   double rms[ntrains];
   
   
   
   
   
   for ( size_t ii = 0; ii < bx.size(); ++ii )
   {
       TFitResultPtr r = h_spec_rate_lumi[ii] -> Fit("gaus","QS");
       h_spec_rate_lumi[ii] -> Write();
       h_bxi1[ii] -> Write();
       h_bxi2[ii] -> Write();
       h_bxlumi[ii]->Write();
       gbx[ii] = double(bx[ii]);
       gbxe[ii] = 0.;
       mean[ii] = r -> Value(1);
       rms[ii]  = r -> Value(2);
       
   }
   
   TGraphErrors * gr_lumi = new TGraphErrors(int(bx.size()),gbx,mean,gbxe,rms);
   
   gr_lumi -> SetMarkerStyle(20);
   gr_lumi -> GetXaxis() -> SetTitle("bx id");
   gr_lumi -> GetYaxis() -> SetTitle("<rate/luminosity>");
   gr_lumi -> GetYaxis() -> SetRangeUser(200,1400);
   gr_lumi -> SetTitle("Four channels");
   //gr_lumi -> Draw("AP");
   TFitResultPtr r = gr_lumi -> Fit("pol0","QS");
   TF1 * fitf = gr_lumi -> GetFunction("pol0");
   double value = r -> Value(0);
   double error = r -> Error(0);
   results[0][0]=value;
   results[0][1]=error;
   
   
   TLegend * leg = new TLegend(0.5,0.7,0.9,0.9);
   leg->SetHeader(Form("Fill %i",fill));
   leg->AddEntry(gr_lumi,"data","p");
   leg->AddEntry(fitf,Form("const fit: %2.2f +- %1.2f", value, error),"l");
   //leg -> Draw();
   
   gr_lumi -> Write();
   
   // PER CHANNEL
   TGraphErrors * gr_ch[48];
   for ( int ch = 0 ; ch < 48 ; ++ch )
   {
       if(ch==40 || ch == 41 || ch == 44 || ch == 45 || ch==3 || ch ==8 || ch==12 || ch ==15 || ch==32){
           for ( size_t ii = 0; ii < bx.size(); ++ii )
           {
               TFitResultPtr r = h_spec_rate_ch[ch][ii] -> Fit("gaus","QS");
               h_spec_rate_ch[ch][ii] -> Write();
               gbx[ii] = double(bx[ii]);
               gbxe[ii] = 0.;
               mean[ii] = r -> Value(1);
               rms[ii]  = r -> Value(2);
               
           }
           gr_ch[ch] = new TGraphErrors(int(bx.size()),gbx,mean,gbxe,rms);
           gr_ch[ch] -> SetName(Form("gr_ch_%i",ch));
           gr_ch[ch] -> SetMarkerStyle(20);
           gr_ch[ch] -> GetXaxis() -> SetTitle("bx id");
           gr_ch[ch] -> GetYaxis() -> SetTitle("<rate/luminosity>");
           gr_ch[ch] -> GetYaxis() -> SetRangeUser(0,800);
           gr_ch[ch] -> SetTitle(Form("Channel %i",ch));
           
           TFitResultPtr r = gr_ch[ch] -> Fit("pol0","QS");
           TF1 * fitf = gr_ch[ch] -> GetFunction("pol0");
           results[ch+1][0]=r->Value(0);
           results[ch+1][1]=r->Error(0);
           gr_ch[ch] -> Write();
       }
   }

   // delete all new stuff!!!
   for ( size_t ii = 0; ii < bx.size(); ++ii )
   {
      delete h_spec_rate_lumi[ii];
      delete h_spec_rate_raw[ii];
      delete h_bxi1[ii];
      delete h_bxi2[ii];
      delete h_bxlumi[ii];
      for ( int j =0 ; j < 48 ; ++j )
         delete h_spec_rate_ch[j][ii];
   }
   delete f;
//   
   delete gr_lumi;
   delete leg;
   for ( int ch = 0 ; ch < 48 ; ++ch )
      delete gr_ch[ch];
   delete fout;
   
}

std::vector<int> AnalysedFills()
{
   // Check if fill already analysed to save time
   TSystemDirectory dir("./", "./");
   TList *files = dir.GetListOfFiles();
   std::vector<int> fillsDone;
   if ( files )
   {
      TSystemFile *file;
      TString fname;
      TIter next(files);
      while ((file=(TSystemFile*)next()))
      {
         fname = file->GetName();
         if (!file->IsDirectory() && fname.BeginsWith("analysis_perch_") && fname.EndsWith(".root")) 
         {
            fillsDone.push_back(TString(fname(15,4)).Atoi());
         }
      }
   }
   return fillsDone;
}   

