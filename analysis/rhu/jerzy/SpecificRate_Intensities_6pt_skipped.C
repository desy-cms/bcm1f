...
void SpecificFill( int fill, string filename,double results[49][2])
{
   ...
   TGraphErrors * gr_lumi = new TGraphErrors(int(bx.size())-6,gbx,mean,gbxe,rms);
...
void SpecificRate(){
   const char * dirname = "/nfs/dust/cms/user/walsh/bril/bcm1f/rhu/";
   ...
   if (files) {
      ...
      while ((file=(TSystemFile*)next())) {
         fname = file->GetName();
         if (!file->IsDirectory() && fname.BeginsWith(Form("rhu_%i_",fill)) && fname.EndsWith(".root") && fill!=5005) {
            filename  = string(dirname)+string(fname.Data());
	    SpecificFill(fill,filename,results);
	    size++;
	    for(int K=0;K<49;++K){
   ...
   for ( int ch = 1 ; ch < 49 ; ++ch )
   {
      ...
      ChannelGraphs[ch] = new TGraphErrors(size,x[ch],y[ch],xerr[ch],yerr[ch]);
