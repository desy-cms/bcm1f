...
void SpecificFill( int fill, string filename,double results[49][2])
{
   ...
   TH1F * h_noise = new TH1F("hnoise","",2000,0,2000);
   TH1F * h_noise_ch[48];
   for ( size_t ii = 0; ii < bx.size(); ++ii )
   {
      ...
      for ( int j =0 ; j < 48 ; ++j )
      {
         h_spec_rate_ch[j][ii] = new TH1F(Form("h_spec_rate_ch_%i_%i",j,bx[ii]),"",1000,0,600);
      }
   }
   ...
   for ( size_t ii = 0; ii < bx.size(); ++ii )
   {
      ...
      mean[ii] = r -> Value(1);
      rms[ii]  = r -> Value(2);
      h_noise->Fill(r->Value(1));
      if(ii==bx.size()-1){
	   results[0][0]=r -> Value(2)/sqrt(r -> Value(1));
   	   results[0][1]=0.;
      }
   }
   ...
   //TFitResultPtr s=h_noise->Fit("gaus","S");
   //results[0][0]=h_noise->GetRMS()/sqrt(h_noise->GetMean());
   //results[0][1]=0.;
   ...
   {
      h_noise_ch[ch]=new TH1F(Form("h_noise_ch%i",ch),"",2000,0,2000);
      if(ch==40 || ch == 41 || ch == 44 || ch == 45){
      for ( size_t ii = 0; ii < bx.size(); ++ii )
      {
         ...
         mean[ii] = r -> Value(1);
         rms[ii]  = r -> Value(2);
	 h_noise_ch[ch]->Fill(r->Value(1));
	 if(ii==bx.size()-1){
	   results[ch+1][0]=r -> Value(2)/sqrt(r -> Value(1));
   	   results[ch+1][1]=0.;
      }
      }
      //results[ch+1][0]=r -> Value(2)/sqrt(r -> Value(1));
      //results[ch+1][0]=h_noise_ch[ch]->GetRMS()/sqrt(h_noise_ch[ch]->GetMean());
      //results[ch+1][1]=0;
      h_noise_ch[ch] -> Write();
      }
   }
...
void SpecificRate(){
   ...
   if (files) {
      ...
      while ((file=(TSystemFile*)next())) {
	    ...
	    for(int K=0;K<49;++K){
		    if(K==0 || K==41 || K==42 || K==45 || K==46){
		    //NORM[K]=results[K][0];
		    x[K][0]=luminosity;
		    y[K][0]=results[K][0];
		    yerr[K][0]=results[K][1];
		    xerr[K][0]=0.;
	    	}
	    }
   ...
   if (files) {
      ...
      while ((file=(TSystemFile*)next())) {
	    ...
	    for(int K=0;K<49;++K){
		    if(K==0 || K==41 || K==42 || K==45 || K==46){
		    x[K][size]=luminosity;
		    y[K][size]=results[K][0];
		    yerr[K][size]=results[K][1];
		    xerr[K][size]=0.;
	    	}
	    }
	    /*
   ...
   FinalGr -> GetYaxis() -> SetTitle("RMS(rate/lumi)/SQRT(Mean rate/lumi)");
   ...
   for ( int ch = 1 ; ch < 49 ; ++ch )
   {
      ...
      ChannelGraphs[ch] -> GetYaxis() -> SetTitle("RMS(rate/lumi)/SQRT(Mean rate/lumi)");
