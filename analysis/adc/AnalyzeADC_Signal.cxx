...
int main(int argc, char* argv[])
{
    ...
    if ( argc < 2 )
    {
        printf("\nUsage: /findPeaks_TestPulse_withCTD_Th3_forWeb <work folder>\n\n"); return 0;
    }
    ...
    for( boost::filesystem::directory_iterator j( input_folder ); j != end_itr; ++j )
    {
        ...
        string OutputFolder ="/localdata/OutputADC/Analysis/";
        
        string filename1 =  j->leaf();
        filename1.erase(filename1.size()-15);
        
        outputfilename = output_folder + "Analysed_"+ j->leaf();
        outputfilename_txt = output_folder + "BaseLinePos.txt";
        ...
        for (int b = 0; b < 6 ; ++b) {
            for (int ch = 0; ch < 8 ; ++ch) {
                ...
                h_Sig_Amp[ch+8*b]=new TH1F(Form("Sig_Amp_b%i_ch%i",b,ch),"Amplitude",256,0,1024);
                h_Sig_Amp[ch+8*b]->GetXaxis()->SetTitle("Signal amplitude (mV)");
...
//        TCanvas *canvas = new TCanvas("canvas","Draw histo",800,600);
        ...
        for (int b = 0; b < 6 ; ++b) {
            for (int ch = 0; ch < 8 ; ++ch) {
                ...
                if (ch+8*b < 50) {
                    //if ( (ch+8*b == 7) ||  (ch+8*b == 15) ||  (ch+8*b == 18) ||  (ch+8*b == 31) ||  (ch+8*b == 33) ||  (ch+8*b == 42) ) {
                    sprintf(tmpstr, "%i    %10.4f    %10.4f    \n", ch+8*b , h_BL[ch+8*b]-> GetMean(), h_BL[ch+8*b]-> GetMeanError());
                    
                    std::string formFormat = output_folder; formFormat += "BaseLinePos_%s_ch%i.png";
                    
                    //h_BL[ch+8*b]-> Get
                    //h_BL[ch+8*b]->Draw();
                    //	     canvas->SaveAs(Form(formFormat.c_str(), filename1.c_str(),ch+8*b));
                    
                    formFormat = output_folder; formFormat += "BaseLineSigma_%s_ch%i.png";
                    
                    //h_BL_sigma[ch+8*b]->Draw();
                    //	     canvas->SaveAs(Form(formFormat.c_str(), filename1.c_str(),ch+8*b));
                    
                    formFormat = output_folder; formFormat += "AmplitudeSpectrum_%s_ch%i.png";
